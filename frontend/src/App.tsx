...
type Asset = { id: string; name: string; relativePath: string; kind: 'image' | 'video'; tags: string[]; untagged: boolean; errors: string[] };
...
export default function App() {
  ...
  const visibleAssets = useMemo(() => assets.filter(asset => {
    ...
    return asset.id === stickyAssetId || (matchingTags && matchingInbox && matchingKind && matchingMissingFolders && (!needle || asset.relativePath.toLowerCase().includes(needle)));
  }), [assets, selectedTags, untagged, mediaKinds, missingGroups, search, groups, tags, stickyAssetId]);
  ...
  return <main className="app">
    ...
    <section className="workspace" style={{ gridTemplateColumns: `210px minmax(350px, 1fr) 8px ${detailsWidth}px` }}>
      ...
      <section className="gallery"><div className="gallery-bar"><span>{visibleAssets.length} assets</span><label className="size-control">Size <input type="range" min="110" max="300" value={cardSize} onChange={event => setCardSize(Number(event.target.value))} /></label>{(selectedTags.length > 0 || missingGroups.length > 0) && <span>AND match</span>}</div><div className="grid" style={{ gridTemplateColumns: `repeat(auto-fill, minmax(${cardSize}px, 1fr))` }}>{visibleAssets.map(asset => <button className={`card ${selected === asset.id ? 'selected' : ''}`} key={asset.id} onClick={() => { setSelected(asset.id); setStickyAssetId(null); }}><div className={`thumb ${asset.kind === 'video' ? 'video-thumb' : ''}`} style={{ height: Math.round(cardSize * 0.78) }}>{asset.kind === 'image' ? <img loading="lazy" src={`${api}/media/${encodeURIComponent(asset.id)}`} alt="" /> : <><img loading="lazy" src={`${api}/thumbnails/${encodeURIComponent(asset.id)}`} alt="" onError={event => { event.currentTarget.style.display = 'none'; }} /><span>▶</span></>}</div><span><i className="media-icon" title={asset.kind}>{asset.kind === 'video' ? '▶' : '▣'}</i>{asset.name}</span>{asset.errors.length > 0 && <b title={asset.errors.join('\n')}>!</b>}</button>)}</div>{visibleAssets.length === 0 && <p className="empty">Nothing matches these filters.</p>}</section>
    ...
    {managerOpen && <div className="modal-backdrop" onMouseDown={() => setManagerOpen(false)}><section className="manager" onMouseDown={event => event.stopPropagation()}><div className="manager-head"><div><p className="eyebrow">LIBRARY TAXONOMY</p><h2>Manage tags</h2></div><button className="secondary" onClick={() => setManagerOpen(false)}>Close</button></div><div className="manager-grid"><section><h3>Folders</h3><p className="helper">Folders can be nested. A tag can belong to several folders.</p>{groups.map(group => <div className="group-row" key={group.id}><input className="group-name" defaultValue={group.name} onBlur={event => updateGroup(group, event.target.value)} /><select value={group.parentId || ''} onChange={event => updateGroup(group, group.name, event.target.value || null)}><option value="">Top level</option>{groups.filter(item => item.id !== group.id).map(option => <option value={option.id} key={option.id}>{option.name}</option>)}</select><button className="delete-small" onClick={() => deleteGroup(group)}>×</button></div>)}<div className="new-group"><input value={newGroupName} onChange={event => setNewGroupName(event.target.value)} placeholder="New folder" /><select value={newGroupParent} onChange={event => setNewGroupParent(event.target.value)}><option value="">Top level</option>{groups.map(group => <option value={group.id} key={group.id}>{group.name}</option>)}</select><button onClick={createGroup}>Add folder</button></div></section><section><div className="tag-list-head"><h3>Tags</h3><button onClick={openNewTag}>New tag</button></div><div className="manage-tag-list">{tags.map(tag => <button key={tag.id} className={draft.id === tag.id ? 'active' : ''} onClick={() => editTag(tag)}><i className="tag-dot" style={{ background: tag.color || '#777' }} />{tag.icon} {tag.name}</button>)}</div></section><section className="tag-form"><h3>{draft.id ? 'Edit tag' : 'New tag'}</h3><label>Name<input value={draft.name} onChange={event => setDraft({ ...draft, name: event.target.value })} placeholder="e.g. Blonde" /></label><label>Icon <span className="helper">any emoji or symbol</span><input value={draft.icon || ''} onChange={event => setDraft({ ...draft, icon: event.target.value })} placeholder="🏷️" /></label><label>Color<input type="color" value={draft.color || '#7c5cff'} onChange={event => setDraft({ ...draft, color: event.target.value })} /></label><fieldset><legend>Folders</legend>{groups.map(group => <label key={group.id}><input type="checkbox" checked={draft.groups.includes(group.id)} onChange={() => setDraft({ ...draft, groups: draft.groups.includes(group.id) ? draft.groups.filter(id => id !== group.id) : [...draft.groups, group.id] })} /> {group.name}</label>)}</fieldset><button onClick={saveTag}>{draft.id ? 'Save tag' : 'Create tag'}</button>{draft.id && <button className="danger delete-tag" onClick={deleteTag}>Delete tag</button>}</section></div></section></div>}
